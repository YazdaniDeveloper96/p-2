...
class IranUniversity:
    ...
    ##################################################################### adding imageBackground for main widget
        ...
        self.enter_city = PhotoImage(file = "img/city_name_botton.png")
        ...
        self.but_home = PhotoImage(file = "img/home.png")
        self.but_setting = PhotoImage(file = "img/setting.png")

        self.note_book=ttk.Notebook(self.main_widget)  
        self.tab_1=Frame(self.note_book,bg='#f0f0f0')                                                                                             
        self.tab_2=Frame(self.note_book,bg='#f0f0f0')
        self.note_book.add(self.tab_1,text='home',image=self.but_home)                                                                                           
        self.note_book.add(self.tab_2,text='settings',image=self.but_setting)
        self.note_book.grid(row=0,column=0)                                                                                        
        self.main_widget.title('Introduction system for universities')                                                                             
    ...
    ################################################################################################ add label in home
        ...
        self.lab_enter_city.grid(row=0,column=0,padx=(25,0),pady=(370,0))
    ################################################################################################ add entrybox in home
        self.entry_box_city=Entry(master=self.canvas1,borderwidth=2,width=15,font=('tahoma',22,font.BOLD)) 
        self.entry_box_city.grid(row=0,column=1,padx=(5,0),pady=(380,10))
    ################################################################################################ add button in home
        ...
        self.button_show.grid(row=0,column=2,padx=(19,0),pady=(370,0))
    ...
    def __show_result(self,event):
        ...
        tree.heading('# 1',text='name of province')
        tree.column('# 2',anchor=CENTER,width=525)
        tree.heading('# 2',text='name of university')
        tree.column('# 3',anchor=CENTER,width=200)
        tree.heading('# 3',text='uni web page')
        ...
        try:
            if city_name=='':
                raise RuntimeError('Please enter city name')
            result=read_api_data(city_name)
            if len(result)==0:
                raise City_Error_Handling('Please enter city name correctly!')
            n=1
            for item in result:
